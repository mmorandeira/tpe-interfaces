class RushAuthForm extends HTMLElement {
    ...
    async init() {
        try {
            ...
            template.innerHTML = `
                <style>${css}</style>
                ${html.replace('<template id="auth-form-template">', '').replace('</template>', '').replace(/<style>.*?<\/style>/s, '')}
            `;
    ...
    setupEventListeners() {
        ...
        this.registerEmail.addEventListener('blur', () => this.validateEmail(this.registerEmail, 'registerEmailError'));
        this.registerName.addEventListener('blur', () => this.validateRequired(this.registerName, 'registerNameError', 'El nombre es requerido'));
        this.registerLastName.addEventListener('blur', () => this.validateRequired(this.registerLastName, 'registerLastNameError', 'El apellido es requerido'));
    ...
    validatePassword() {
        ...
        if (!hasNumber || !hasSpecial) {
            this.showError(this.registerPassword, errorElement, 'Debe contener 1 número y 1 carácter especial');
            return false;
        }
    ...
    handleLogin(e) {
        ...
        const passwordValid = this.validateRequired(this.loginPassword, 'loginPasswordError', 'La contraseña es requerida');
    ...
    handleRegister(e) {
        ...
        const validations = [
            ...
            this.validateRequired(this.registerLastName, 'registerLastNameError', 'El apellido es requerido'),
        ...
        this.dispatchEvent(new CustomEvent('registerSubmit', {
            detail: formData,
            bubbles: true
        }));
    ...
    handleSocialAuth(e) {
        ...
        this.dispatchEvent(new CustomEvent('socialAuth', {
            detail: { provider },
            bubbles: true
        }));
